...
RSpec.describe FuzzyTimeAgo do
  it "has a version number" do
    expect(FuzzyTimeAgo::VERSION).not_to be nil
  end
  ...
  describe ".fuzzy_ago" do
    ...
    it "returns 'about a month ago' for 4 weeks in a non-leap-year February" do
      Timecop.freeze Time.new(2025,03,01,12,0,0)
      expect(fuzzy_ago_from_now(28 * DAYS)).to eq("about a month ago")
    end

    it "returns 'about a month ago' for 1-2 months", :focus do      
    ...
    it "returns years for 10+ years" do
      expect(FuzzyTimeAgo.fuzzy_ago(today.prev_year(10))).to eq("about 10 years ago")
      expect(FuzzyTimeAgo.fuzzy_ago(today.prev_year(20))).to eq("about 20 years ago")
      expect(FuzzyTimeAgo.fuzzy_ago(today.prev_year(50))).to eq("about 50 years ago")
    end
  end

  context "when given invalid input" do
    it "raises an ArgumentError for unsupported types" do
      expect { FuzzyTimeAgo.fuzzy_ago("invalid") }.to raise_error(ArgumentError, "Expected Time or Date object")
    end
  end
